...
;; @deffn {Procedure} remove-comments sx0 => sx1
;; Remove comments from the parse tree @var{sx0} generated by @code{parse-c99}.
;; @end deffn
(define (remove-comments tree)
  (define (rem-com sx)
    (match sx
      ('() sx)
      (`((@ (comment . ,_)) . ,tail) (rem-com tail))
      (`((@ (comment . ,_) . ,rest) . ,tail) `((@ rest) . ,(rem-com tail)))
      (`((comment . ,_) . ,tail) (rem-com tail))
      ((head . tail) (cons (rem-com head) (rem-com tail)))
      (atom atom)))
  (rem-com tree))
