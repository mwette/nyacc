...
(define (mtail->bsdesc mtail)
  (let ((defined (*defined*)))
    (match mtail
      ...
      (`((bit-field ,size) . ,rest)
       `(bs:bitfield ,(mtail->bsdesc rest) ,(const-expr->number size)))
      (`((extern) . ,rest) (mtail->bsdesc rest))
      (__
       (sx-match (car mtail)
         ((typename ,name)
          (let ((name (rename name 'type)))
         ...
         ((struct-def (@ . ,attr) (field-list . ,fields))
          (let ((fields (cnvt-fields fields mtail->bsdesc)))
            (if (packed? attr)
                `(bs:struct (list ,@fields) #t)
                `(bs:struct (list ,@fields)))))
         ((struct-ref (ident ,struct-name))
          (strings->symbol "struct-" struct-name))
         ((union-def (ident ,name) ,field-list)
          (mtail->bsdesc `((union-def ,field-list))))
         ((union-def (field-list . ,fields))
          `(bs:union (list ,@(cnvt-fields fields mtail->bsdesc))))
         ((union-ref (ident ,name)) (strings->symbol "union-" name))
         ((enum-def (@ . ,attr) (ident ,ident) ,rest)
          (mtail->bsdesc `((enum-def (@ . ,attr) ,rest))))
         ((enum-def (@ . ,attr) ,edl)
...
(define (unwrap-mdecl mdecl)
  (let ((defined (*defined*))
    ...
    (match (car mtail)
      ...
      (`(union-ref (ident ,name)) mname)
      (`(array-of ,size) `(arg->pointer ,mname))
      (`(array-of) `(arg->pointer ,mname))
      ;; not expected
      (`(void) #f)
      (`(struct-def . ,_1) `(bytestructure-ref ,mname))
      (`(union-def . ,_1) `(bytestructure-ref ,mname))
      (otherwise (fherr "unwrap-mdecl: missed:\n~A" (ppstr mtail))))))
...
(define* (defined-type-wrapper name mname)
  (let* ((udecl (expand-typerefs
    ...
    (match (md-tail mdecl)
      ...
      (__ `(bytestructure ,(string->symbol name) ,mname)))))
...
(define (wrap-mdecl mdecl)
  (let ((defined (*defined*))
    ...
    (match mtail
      ...
      (otherwise (fherr "wrap-mdecl missed:\n~A" (ppstr mtail))))))
        ...
        (match mtail
          ...
          (`((array-of ,dim) . ,rest)
           (values
            (dcons name defined)
            (xcons* seed
              (deftype type (mtail->bsdesc mtail))
           ...
           (sx-match (car mtail)
             ...
             (((fixed-type float-type) ,basename)
              (values (dcons name defined)
                      (cons (deftype type (mtail->bsdesc mtail)) seed)))
     ...
     ((sx-match tspec
        ...
        ((struct-def (@ . ,aggr-attr) (ident ,agname) ,field-list)
         (let* ((agname (rename agname 'type))
                ...
                (sflds (cnvt-fields (sx-tail field-list) mtail->bsdesc))
                (agdef (if (packed? aggr-attr)
                           `(bs:struct (list ,@sflds) #t)
                           `(bs:struct (list ,@sflds)))))
           (cond
            ((bkref-getall attr) =>
             (lambda (name-list)
               (fold-values
                ...
                (xcons* seed
                  (deftype atype agdef)
                  (deftype atype* `(bs:pointer ,atype))))))
