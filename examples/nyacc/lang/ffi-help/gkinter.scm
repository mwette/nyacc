...
(grid (ttk:entry .c.feet #:width 7 #:textvariable feet)
      #:column 2 #:row 1 #:sticky we)
(grid (ttk:label .c.meters #:textvariable meters)
      #:column 2 #:row 2 #:sticky we)
(grid (ttk:button .c.calc #:text "Calculate" #:command calculate)
      #:column 3 #:row 3 #:sticky w)

(grid (ttk:label .c.flbl #:text "feet")
      #:column 3 #:row 1 #:sticky w)
(grid (ttk:label .c.islbl #:text "is equivalent to")
      #:column 1 #:row 2 #:sticky e)
(grid (ttk:label .c.mlbl #:text "meters")
      #:column 3 #:row 2 #:sticky w)

(for-each
 (lambda (w) (grid configure $w #:padx 5 #:pady 5))
 (winfo children .c))
(focus .c.feet)
(bind . <Return> {calculate})
