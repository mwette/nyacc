...
(define-public (make-for lvar expr body dict)
  (let* ((toplev? (eq? 'toplevel (car lvar)))
	 ...
	 (next `(call ,(xlib-ref 'oct:iter-next) ,rval ,ivar)) ; ???
	 (ilsym (genxsym "iloop"))
	 (olsym (genxsym "oloop"))
...
(define (xlang-sxml->xtil exp env opts)
	...
	'() dict))
	 ...
	 (values tree '() dict)))
      ...
      ((assn (@ . ,attr) (ident ,name) ,rhsx)	; assign variable
       (let* ((ref (lookup name dict))
	      (dict (if ref dict (add-symbol name dict))))
	 ;;(sferr "dict:\n") (pperr dict)
       ...
       (let* ((lval-expl
		 ...
		 (if (null? elts) '()
		     (let* ((n (string-append "arg" (number->string ix)))
		       ...
		       (cons
			(sx-match (car elts)
			  ((ident ,name) `(var-assn (ident ,name) ,rv))
			  ((aref-or-call ,ax ,xl) `(elt-assn ,ax ,xl ,rv))
			  ((sel (ident ,name) ,expr) `(mem-assn ,expr ,name ,rv))
  ...
  (define (fU tree seed dict kseed kdict) ;; => seed dict
	...
	(let* ((tail (rtail kseed))
	       ...
	       (stmt `(call ,(xlib-ref 'oct:elt-assn) ,aexp ,expl ,rhsx)))
	...
	(let* ((tail (rtail kseed))
	       ...
	       (rhsx (list-ref tail 2))
	       (stmt `(call ,(xlib-ref 'oct:struct-set!) ,expr ,name ,rhsx)))
	  (values (cons stmt seed) kdict)))
	...
	;;       (set! a $arg0)
	;;       (oct:elt-assn b 1 $arg1)
	;;       (oct:mem-assn c "n" $arg2)))
	(let* ((body (car kseed))
	...
	(let* ((tail (rtail kseed))
	  ...
	  (values (cons `(call ,(xlib-ref 'make-oct:range) ,lb ,inc ,ub) seed)
		  kdict)))

       ((or) (make-opcall 'oct:or seed kseed kdict))
       ((and) (make-opcall 'oct:and seed kseed kdict))
       ((eq) (make-opcall 'oct:eq seed kseed kdict))
       ((ne) (make-opcall 'oct:ne seed kseed kdict))
       ((lt) (make-opcall 'oct:lt seed kseed kdict))
       ((gt) (make-opcall 'oct:gt seed kseed kdict))
       ((le) (make-opcall 'oct:le seed kseed kdict))
       ((ge) (make-opcall 'oct:ge seed kseed kdict))
       
       ((add) (make-opcall 'oct:+ seed kseed kdict))
       ((sub) (make-opcall 'oct:- seed kseed kdict))
       ((dot-add) (make-opcall 'oct:.+ seed kseed kdict))
       ((dot-sub) (make-opcall 'oct:.- seed kseed kdict))
       ((mul) (make-opcall 'oct:* seed kseed kdict))
       ((div) (make-opcall 'oct:/ seed kseed kdict))
       ((ldiv) (make-opcall 'oct:\ seed kseed kdict))
       ((pow) (make-opcall 'oct:^ seed kseed kdict))
       ((dot-mul) (make-opcall 'oct:.* seed kseed kdict))
       ((dot-div) (make-opcall 'oct:./ seed kseed kdict))
       ((dot-pow) (make-opcall 'oct:.^ seed kseed kdict))
       
       ((neg) (make-opcall 'oct:neg seed kseed kdict))
       ((pos) (make-opcall 'oct:pos seed kseed kdict))
       ((not) (make-opcall 'oct:not seed kseed kdict))
       
       ((transpose) (make-opcall 'oct:xpose seed kseed kdict))
       ((conj-transpose) (make-opcall 'oct:cj-xpose seed kseed kdict))

       ;; aref-or-call
       ((aref-or-call)
	(let ((proc-or-array (cadr kseed)) (args (cdar kseed)))
	  (values
	   (cons `(call ,(xlib-ref 'oct:aref-or-call) ,proc-or-array ,@args)
		 seed)
	   kdict)))

       ((sel)
	(values
	 (cons `(call ,(xlib-ref 'oct:struct-ref) ,(car kseed)
	...
	;;   (oct:array-set-row! 0 (list a00 a01 ... a0N))
	;;   ...
	;;   (oct:array-set-row! M (list aM0 aM1 ... aMN))
	;;   $aval)
	 (let* ((tail (rtail kseed))
		(row1 (car tail))
		...
		(body (let loop ((ix 0) (rows tail))
			(if (null? rows) aval
			    `(seq (call ,(xlib-ref 'oct:array-set-row!)
					,aval (const ,ix)
