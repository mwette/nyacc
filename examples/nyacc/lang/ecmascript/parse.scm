...
(define RegexpLiteral 'RegexpLiteral)
   ...
   ;;
    ...
    (FunctionDeclaration
     (function Identifier lparen rparen lbrace FunctionBody rbrace
	       ($$ `(var (,$2 (lambda () ,$6)))))
     (function Identifier lparen FormalParameterList rparen lbrace
	       FunctionBody rbrace ($$ `(var (,$2 (lambda ,$4 ,$7)))))
     )
    (FunctionExpression
     (function lparen rparen lbrace FunctionBody rbrace ($$ `(lambda () ,$5)))
     (function Identifier lparen rparen lbrace FunctionBody rbrace
	       ($$ `(lambda () ,$6)))
     (function lparen FormalParameterList rparen lbrace FunctionBody rbrace
	       ($$ `(lambda ,$3 ,$6)))
     (function Identifier lparen FormalParameterList rparen lbrace
	       FunctionBody rbrace ($$ `(lambda ,$4 ,$7)))
     )
    (FormalParameterList
     (Identifier ($$ `(,$1)))
    ...
    (IterationStatement
     (do Statement while lparen Expression rparen semicolon ($$ `(do ,$2 ,$5)))
     ...
     (for lparen ExpressionNoIn semicolon Expression semicolon rparen Statement
	  ($$ `(for ,$3 ,$5 #f ,$8)))
     (for lparen ExpressionNoIn semicolon Expression semicolon Expression rparen
	  Statement ($$ `(for ,$3 ,$5 ,$7 ,$9)))

     (for lparen var VariableDeclarationListNoIn semicolon semicolon rparen
	  Statement ($$ `(for (var ,@$4) #f #f ,$8)))
     (for lparen var VariableDeclarationListNoIn semicolon semicolon Expression
	  rparen Statement ($$ `(for (var ,@$4) #f ,$7 ,$9)))
     (for lparen var VariableDeclarationListNoIn semicolon Expression semicolon
	  rparen Statement ($$ `(for (var ,@$4) ,$6 #f ,$9)))
     (for lparen var VariableDeclarationListNoIn semicolon Expression semicolon
	  Expression rparen Statement ($$ `(for (var ,@$4) ,$6 ,$8 ,$10)))
    ...
    (PropertyNameAndValueList
     (PropertyName colon AssignmentExpression ($$ `((,$1 ,$3))))
    ...
    (CallExpression
     (MemberExpression Arguments ($$ `(call ,$1 ,$2)))
    ...
    (ConditionalExpression
     ...
     (LogicalORExpression ? AssignmentExpression colon AssignmentExpression
			  ($$ `(if ,$1 ,$3 ,$5))))
    (ConditionalExpressionNoIn
     (LogicalORExpressionNoIn ($$ $1))
     (LogicalORExpressionNoIn ? AssignmentExpressionNoIn colon
			      AssignmentExpressionNoIn ($$ `(if ,$1 ,$3 ,$5))))
