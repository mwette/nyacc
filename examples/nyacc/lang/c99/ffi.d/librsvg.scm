...
(define RsvgHandle-desc
  (bs:struct
    (list `(parent
          ...
          `(priv ,(bs:pointer "RsvgHandlePrivate"))
          `(_abi_padding
             ,(bs:vector 15 (bs:pointer intptr_t))))))
(export RsvgHandle-desc)
(define-fh-compound-type/p RsvgHandle RsvgHandle-desc)
(define struct-_RsvgHandle RsvgHandle)

;; typedef struct RsvgHandlePrivate RsvgHandlePrivate;
(define-fh-pointer-type RsvgHandlePrivate*)
...
;;   gpointer _abi_padding[15];
;; };
(define RsvgHandleClass-desc
  (bs:struct
    (list `(parent
             ,(bs:struct
                (list `(g_type_class ,GTypeClass-desc)
                      `(construct_properties ,(bs:pointer "GSList"))
                      `(constructor ,(bs:pointer intptr_t))
                      `(set_property ,(bs:pointer intptr_t))
                      `(get_property ,(bs:pointer intptr_t))
                      `(dispose ,(bs:pointer intptr_t))
                      `(finalize ,(bs:pointer intptr_t))
                      `(dispatch_properties_changed
                         ,(bs:pointer intptr_t))
                      `(notify ,(bs:pointer intptr_t))
                      `(constructed ,(bs:pointer intptr_t))
                      `(flags ,gsize-desc)
                      `(pdummy ,(bs:vector 6 gpointer-desc)))))
          `(_abi_padding
             ,(bs:vector 15 (bs:pointer intptr_t))))))
(export RsvgHandleClass-desc)
(define-fh-compound-type/p RsvgHandleClass RsvgHandleClass-desc)
(define struct-_RsvgHandleClass RsvgHandleClass)
...
(define-fh-compound-type/p RsvgDimensionData RsvgDimensionData-desc)
(define struct-_RsvgDimensionData RsvgDimensionData)
...
;;   int y;
;; };
(define RsvgPositionData-desc
  (bs:struct (list `(x ,int) `(y ,int))))
(export RsvgPositionData-desc)
(define-fh-compound-type/p RsvgPositionData RsvgPositionData-desc)
(define struct-_RsvgPositionData RsvgPositionData)
...
(define rsvg_handle_new_from_data
  (let ((~f (ffi:pointer->procedure
    ...
    (lambda (data data_len error)
      (let ((~data_len (unwrap-gsize data_len))
            (~error (unwrap~pointer error)))
  ...
  (let ((~f (ffi:pointer->procedure
    ...
    (lambda (handle size_func user_data user_data_destroy)
      (let ((~handle (unwrap-RsvgHandle* handle))
            ...
            (~user_data_destroy
              (unwrap-GDestroyNotify user_data_destroy)))
        (~f ~handle
            ~size_func
            ~user_data
            ~user_data_destroy)))))
...
(define rsvg_pixbuf_from_file_at_zoom
  (let ((~f (ffi:pointer->procedure
    ...
    (lambda (file_name x_zoom y_zoom error)
      (let ((~x_zoom (unwrap~float x_zoom))
...
(define rsvg_pixbuf_from_file_at_size
  (let ((~f (ffi:pointer->procedure
    ...
    (lambda (file_name width height error)
      (let ((~width (unwrap-gint width))
            (~height (unwrap-gint height))
            (~error (unwrap~pointer error)))
...
(define rsvg_pixbuf_from_file_at_max_size
  (let ((~f (ffi:pointer->procedure
    ...
    (lambda (file_name max_width max_height error)
      (let ((~max_width (unwrap-gint max_width))
            (~max_height (unwrap-gint max_height))
            (~error (unwrap~pointer error)))
...
(define rsvg_pixbuf_from_file_at_zoom_with_max
  (let ((~f (ffi:pointer->procedure
    ...
    (lambda (file_name
      ...
      (let ((~x_zoom (unwrap~float x_zoom))
            (~y_zoom (unwrap~float y_zoom))
            (~max_width (unwrap-gint max_width))
            (~max_height (unwrap-gint max_height))
            (~error (unwrap~pointer error)))
        (~f file_name
            ~x_zoom
