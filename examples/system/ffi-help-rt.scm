...
(define-module (system ffi-help-rt)
  #:export (fh-type?
	    ...
	    ;; debugging
	    fht-unwrap fht-pointer-to fht-value-at
	    make-bs-printer
	    ;; deprecated
	    )
...
;; ----------------------------------------------------------------------------
;; bytestructure function pseudo-type only pointers are used
...
(define (undefined-procedure)
  (error "uninitialized procedure"))

;; @deffn {Syntax} define-fh-function*-type name return-type arg-types
;; Define wrapper and unwrapper for a function pointer.
;; make can take a lambda OR a pointer
;; @end deffn
...
;; @deffn {Procecure} fh-cast type expr
...
;; @end deffn
#;(define-syntax-rule (define-fh-base-type type desc pred make)
  (begin
    ;;(define-public type
    (define-fh-compound-type type ...))
  ;;(make-fht name unwrap wrap pointer-to value-at printer)
  )
    ...
    (syntax-case x ()
	 ...
	 #'(begin
		...
		(() (make-struct/no-tail type (default-val ffi-type)))))
	     (define type
	       (make-fht (quote type) unwrap wrap
...
(define-syntax OLD-define-fh-function
  (lambda (x)
    (syntax-case x ()
...
(define-syntax OLD-define-fh-function/p
  (lambda (x)
    (syntax-case x ()
      ((_ name return-t args-t)
       (with-syntax ((wrap (gen-id #'name "wrap-" #'name))
...
(define-syntax OLD-define-fh-fixed/p
  (lambda (x)
    (syntax-case x ()
      ((_ type desc)
       (with-syntax ((unwrap* (gen-id #'type "unwrap-" #'type "*"))
...
(define-syntax OLD-define-fh-float/p
  (lambda (x)
    (syntax-case x ()
      ((_ type desc)
       (with-syntax ((unwrap* (gen-id #'type "unwrap-" #'type "*"))
